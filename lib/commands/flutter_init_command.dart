...
class FlutterInitCommand {
  ...
  Future<void> run() async {
    try {
      ...
      // 실제 sample 폴더 경로 찾기
      // bin/naeileun.dart에서 실행되므로, 상위 폴더의 sample 디렉토리를 찾음
      final scriptPath = Platform.script.toFilePath();
      final binDir = path.dirname(scriptPath);
      final projectRoot = path.dirname(binDir);
      final actualSamplePath = path.join(projectRoot, 'sample');

      if (!Directory(actualSamplePath).existsSync()) {
        logger.err('❌ sample 폴더를 찾을 수 없습니다: $actualSamplePath');
        exit(1);
      }

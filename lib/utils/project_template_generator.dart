...
''';
  ...
  /// lib/main/bootstrap_mobile.dart 생성
  Future<void> _createBootstrapMobile(String targetPath, String packageName, Logger logger) async {
    final file = File(path.join(targetPath, 'lib', 'main', 'bootstrap_mobile.dart'));
    await file.parent.create(recursive: true);
...
import 'package:$packageName/app/routes/app_pages.dart';
import 'package:$packageName/app/services/auth_service.dart';
import 'package:$packageName/app/theme/custom_theme.dart';
...
''';
  ...
  /// app/routes/app_routes.dart 생성
  Future<void> _createAppRoutes(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'routes', 'app_routes.dart'));
    await file.parent.create(recursive: true);
...
''';
  ...
  /// app/routes/app_pages.dart 생성
  Future<void> _createAppPages(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'routes', 'app_pages.dart'));
    await file.parent.create(recursive: true);
...
''';
  ...
  /// splash 페이지 생성
  Future<void> _createSplashPage(String targetPath, String packageName) async {
    // controller
...
''';
    ...
    // view
    final viewFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'splash', 'views', 'splash_view.dart'),
    );
...
''';
    ...
    final bindingFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'splash', 'bindings', 'splash_binding.dart'),
    );
...
''';
  ...
  /// login 페이지 생성
  Future<void> _createLoginPage(String targetPath, String packageName) async {
    // controller
    final controllerFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'login', 'controllers', 'login_controller.dart'),
    );
    await controllerFile.parent.create(recursive: true);
    final controllerContent =
        '''import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:$packageName/app/models/auth_data.dart';
import 'package:$packageName/app/repositories/auth_repository.dart';
import 'package:$packageName/app/routes/app_pages.dart';
import 'package:$packageName/app/services/auth_service.dart';
...
''';
    ...
    // view
    final viewFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'login', 'views', 'login_view.dart'),
    );
    await viewFile.parent.create(recursive: true);
    final viewContent =
        '''import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:get/get.dart';
import 'package:$packageName/app/pages/login/controllers/login_controller.dart';
...
''';
    ...
    // binding
    final bindingFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'login', 'bindings', 'login_binding.dart'),
    );
    await bindingFile.parent.create(recursive: true);
    final bindingContent = '''import 'package:get/get.dart';
...
''';
  ...
  /// home 페이지 생성
  Future<void> _createHomePage(String targetPath, String packageName) async {
    // controller
    final controllerFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'home', 'controllers', 'home_controller.dart'),
    );
    await controllerFile.parent.create(recursive: true);
...
''';
    ...
    // view
    final viewFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'home', 'views', 'home_view.dart'),
    );
    await viewFile.parent.create(recursive: true);
    final viewContent = '''import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
...
''';
    ...
    // binding
    final bindingFile = File(
      path.join(targetPath, 'lib', 'app', 'pages', 'home', 'bindings', 'home_binding.dart'),
    );
    await bindingFile.parent.create(recursive: true);
    final bindingContent = '''import 'package:get/get.dart';
...
''';
  ...
  /// AuthService 생성
  Future<void> _createAuthService(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'services', 'auth_service.dart'));
    await file.parent.create(recursive: true);
...
import 'package:$packageName/app/models/auth_data.dart';
import 'package:$packageName/app/repositories/auth_repository.dart';
import 'package:$packageName/app/utils/secure_storage.dart';
...
''';
  ...
  /// AuthRepository 생성
  Future<void> _createAuthRepository(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'repositories', 'auth_repository.dart'));
    await file.parent.create(recursive: true);

    final content =
        '''import 'package:$packageName/app/data/auth_data.dart';
import 'package:$packageName/app/data/dto/login_dto.dart';
import 'package:$packageName/app/models/auth_data.dart';
...
''';
  ...
  /// AuthData 생성
  Future<void> _createAuthData(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'data', 'auth_data.dart'));
    await file.parent.create(recursive: true);

    final content = '''import 'package:$packageName/app/utils/api.dart';
...
''';
  ...
  /// LoginDto 생성
  Future<void> _createLoginDto(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'data', 'dto', 'login_dto.dart'));
    await file.parent.create(recursive: true);

    final content = '''import 'package:$packageName/app/models/auth_data.dart';
...
''';
  ...
  /// AuthDataModel 생성
  Future<void> _createAuthDataModel(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'models', 'auth_data.dart'));
    await file.parent.create(recursive: true);
...
''';
  ...
  /// API Util 생성
  Future<void> _createApiUtil(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'utils', 'api.dart'));
    await file.parent.create(recursive: true);

    final content =
        '''// TODO: 백엔드 API 연동 시 아래 import 주석 해제
// import 'dart:convert';
...
''';
  ...
  /// SecureStorage 생성
  Future<void> _createSecureStorage(String targetPath, String packageName, bool hasWeb) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'utils', 'secure_storage.dart'));
    await file.parent.create(recursive: true);
...
import 'package:$packageName/app/utils/storage_interface.dart';
import 'package:$packageName/app/utils/mobile_storage_adapter.dart';
import 'package:$packageName/app/utils/web_storage_stub.dart'
    if (dart.library.html) 'package:$packageName/web/utils/web_storage_adapter.dart';
...
''';
  ...
  /// StorageInterface 생성
  Future<void> _createStorageInterface(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'utils', 'storage_interface.dart'));
    await file.parent.create(recursive: true);
...
''';
  ...
  /// WebStorageStub 생성
  Future<void> _createWebStorageStub(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'utils', 'web_storage_stub.dart'));
    await file.parent.create(recursive: true);

    final content = '''import 'package:$packageName/app/utils/storage_interface.dart';
...
''';
  ...
  Future<void> _createPermissionService(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'utils', 'permission_service.dart'));
    await file.parent.create(recursive: true);
...
''';
  ...
  /// CustomTheme 생성
  Future<void> _createCustomTheme(String targetPath, String packageName) async {
    final file = File(path.join(targetPath, 'lib', 'app', 'theme', 'custom_theme.dart'));
    await file.parent.create(recursive: true);

    // 파일이 길어서 직접 작성하는 대신, sample 폴더에서 읽어서 생성
    // 하지만 sample 폴더를 사용하지 않으므로 여기에 전체 내용을 포함
    // 간단한 버전으로 생성
